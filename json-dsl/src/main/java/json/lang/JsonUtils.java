...
public class JsonUtils {
     ...
     * @return
     */
    public static Map<String, java.lang.Object> unmodifiableMap(JsonObject object) {
        if (object == null)
            return null;
        ...
        for (JsonMember<?> member : object.getValue()) {
            ...
            case ARRAY:
                map.put(member.getName(), unmodifiableList((JsonArray) value));
                break;

            case OBJECT:
                map.put(member.getName(), unmodifiableMap((JsonObject) value));
                break;
